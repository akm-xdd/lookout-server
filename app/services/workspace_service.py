...
class WorkspaceService:
    ...
    async def _get_user_workspace_count(self, user_id: str) -> int:
        ...
        try:
            response = self.supabase.table("workspaces").select("id", count="exact").eq("user_id", user_id).execute()
    ...
    async def _validate_workspace_name_unique(self, name: str, user_id: str, exclude_id: Optional[UUID] = None) -> None:
        ...
        try:
            query = self.supabase.table("workspaces").select("id").eq("user_id", user_id).eq("name", name)
            ...
            # Don't fail validation on database errors, just log
            print(f"Warning: Could not validate workspace name uniqueness: {e}")

    async def get_user_workspaces(self, user_id: str) -> List[WorkspaceResponse]:
        """Get all workspaces for a user"""
        try:
            response = self.supabase.table("workspaces").select("*").eq("user_id", user_id).order("created_at", desc=False).execute()
    ...
    async def get_workspace(self, workspace_id: UUID, user_id: str) -> Optional[WorkspaceResponse]:
        ...
        try:
            response = self.supabase.table("workspaces").select("*").eq("id", str(workspace_id)).eq("user_id", user_id).execute()
    ...
    async def update_workspace(self, workspace_id: UUID, workspace_data: WorkspaceUpdate, user_id: str) -> Optional[WorkspaceResponse]:
        ...
        try:
            ...
            response = self.supabase.table("workspaces").update(update_data).eq("id", str(workspace_id)).eq("user_id", user_id).execute()
    ...
    async def delete_workspace(self, workspace_id: UUID, user_id: str) -> bool:
        ...
        try:
            ...
            response = self.supabase.table("workspaces").delete().eq("id", str(workspace_id)).eq("user_id", user_id).execute()
    ...
    async def get_workspace_stats(self, workspace_id: UUID, user_id: str) -> dict:
        ...
        try:
            ...
            endpoint_response = self.supabase.table("endpoints").select("id", count="exact").eq("workspace_id", str(workspace_id)).execute()
            endpoint_count = endpoint_response.count or 0
            
            # Get active endpoint count
            active_response = self.supabase.table("endpoints").select("id", count="exact").eq("workspace_id", str(workspace_id)).eq("is_active", True).execute()
